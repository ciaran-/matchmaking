...
describe('deriveUniqueUsername', () => {
	...
	it('uses the Clerk username when set', async () => {
		...
		const result = await deriveUniqueUsername(user as unknown as ClerkUser);
	...
	it('throws after 5 failed retries', async () => {
		...
		mockFindUnique.mockResolvedValue(makeDbUser() as unknown as DbUser);

		const user = makeClerkUser({ username: 'alice' });
		await expect(
			deriveUniqueUsername(user as unknown as ClerkUser),
		).rejects.toThrow('Could not derive a unique username');
	});
});
...
describe('syncUser', () => {
	...
	it('returns findUnique result and skips upsert when db_synced cookie matches', async () => {
		...
		mockFindUnique.mockResolvedValueOnce(dbUser as unknown as DbUser);
	...
	it('runs upsert and sets cookie when no db_synced cookie is present', async () => {
		...
		mockFindUnique.mockResolvedValueOnce(null); // username not taken
		const dbUser = makeDbUser();
		mockUpsert.mockResolvedValueOnce(dbUser as unknown as DbUser);
	...
	it('handles race condition: returns existing row when upsert throws P2002', async () => {
		...
		mockFindUnique
			...
			.mockResolvedValueOnce(existingRow as unknown as DbUser); // existing row found after race
